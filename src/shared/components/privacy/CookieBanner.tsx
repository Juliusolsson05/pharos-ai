    ...
    <div className="fixed inset-x-0 bottom-0 z-[80] px-2 pb-2 sm:px-3 sm:pb-3">
      <div className="mx-auto flex max-w-5xl flex-col gap-2 rounded-lg border border-[var(--bd)] bg-[color:color-mix(in_srgb,var(--bg-1)_92%,black)] px-3 py-2 shadow-2xl shadow-black/25 backdrop-blur sm:flex-row sm:items-center sm:justify-between sm:gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-[11px] leading-4 text-[var(--t2)] sm:text-xs">
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          <Button
            variant="outline"
            size="xs"
            className="border-[var(--bd)] bg-[var(--bg-2)] text-[10px] text-[var(--t2)] hover:bg-[var(--bg-3)] hover:text-[var(--t1)]"
          ...
          <Button
            variant="outline"
            size="xs"
            className="border-[var(--bd)] bg-[var(--bg-2)] text-[10px] text-[var(--t1)] hover:bg-[var(--bg-3)]"
            onClick={onManage}
          >
            Manage
          </Button>
          <Button
            size="xs"
            className="bg-[var(--blue)] text-[10px] text-[var(--t1)] hover:bg-[var(--blue-l)]"
            onClick={onAcceptAll}
          >
            Accept all
          </Button>

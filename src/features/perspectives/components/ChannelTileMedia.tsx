    ...
    <div className="relative aspect-video bg-[var(--bg-app)]">
      ...
      {status === 'loading' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[var(--bg-app)] text-center">
          ...
          <span className="mono text-[9px] tracking-wider text-[var(--t4)]">CHECKING LIVE STATUS</span>
        </div>
      )}
      {status === 'offline' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[var(--bg-app)] text-center">
          <Slash size={18} className="text-[var(--t4)]" strokeWidth={1.5} />
          <span className="mono text-[9px] tracking-wider text-[var(--t3)]">CHANNEL OFFLINE</span>
          <span className="px-2 text-[9px] text-[var(--t4)]">No active live stream detected right now.</span>
        </div>
      )}
      {isLive && !canEmbedLive && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[var(--bg-app)] text-center">
          <TvMinimalPlay size={18} className="text-[var(--blue-l)]" strokeWidth={1.5} />
          <span className="mono text-[9px] tracking-wider text-[var(--t2)]">LIVE ON YOUTUBE</span>
          <span className="px-2 text-[9px] text-[var(--t4)]">This channel is live, but YouTube does not allow embedding it here.</span>
        </div>
      )}
      {isLive && canEmbedLive && !renderEmbed && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[var(--bg-app)] text-center">
          <TvMinimalPlay size={18} className="text-[var(--blue-l)]" strokeWidth={1.5} />
          <span className="mono text-[9px] tracking-wider text-[var(--t2)]">LIVE PREVIEW AVAILABLE</span>
          <span className="px-2 text-[9px] text-[var(--t4)]">Tap play to open the live embed here.</span>
        </div>
      )}
      {isLive && canEmbedLive && !renderEmbed && (
        <Button
      ...
      {showEmbed && !isFocused && (
        ...
        >
          <span className="mono text-[10px] tracking-wider">CLICK TO FOCUS</span>
        </Button>
      )}
      {showEmbed && isFocused && (
        <div className="pointer-events-none absolute left-2 top-2 flex items-center gap-2 border border-[var(--blue-l)] bg-[var(--bg-app)]/90 px-2 py-1">
          ...
          <span className="mono text-[8px] text-[var(--blue-l)]">FOCUSED - SOUND ON</span>
        </div>
      )}
      {status !== 'loading' && !isFocused && (
        <div className="pointer-events-none absolute right-2 top-2">
          <span className={`border px-1.5 py-0.5 text-[7px] mono font-bold ${
            isLive

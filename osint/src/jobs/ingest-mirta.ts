...
export async function processMirtaIngest(job: Job) {
  ...
  // Derive map features
  const installations = buildInstallations(sites);
  ...
  if (installations.length > 0) {
    await prisma.mapFeature.createMany({
      data: installations.map((inst) => ({
        ...
        properties: { name: inst.name, country: inst.country, state: inst.state, isJoint: inst.isJoint },
        source: SOURCE,
      })),
    });
  }
  ...
  await job.log(`Done: ${stored} sites, ${installations.length} active installations in ${Date.now() - start}ms`);
  return { status: 'ok', totalSites: sites.length, stored, activeInstallations: installations.length, durationMs: Date.now() - start };
}
